...
describe("Matrix (dynamic)", () => {
  ...
  it("constructs from array-like", () => {
    const m = Float32Matrix2x2.fromValues([1, 2, 3, 4]);
    expect(Array.from(m)).toEqual([1, 2, 3, 4]);
  });
  ...
  it("multiplies matrices", () => {
    ...
    const p = m.matMul(n);
    expect(Array.from(p)).toEqual([19, 22, 43, 50]);
  });
  ...
  it("multiplies matrices with different shapes", () => {
    ...
    expect(Array.from(p)).toEqual([21, 24, 27, 47, 54, 61]);
  });

  it("constructs from nested arrays via fromValues", () => {
    const m = Float32Matrix2x2.fromValues([
      [1, 2],
      [3, 4],
    ]);
    expect(Array.from(m)).toEqual([1, 2, 3, 4]);
  ...
  it("getN/setN works with coordinates", () => {
    ...
    expect(Array.from(m)).toEqual([0, 0, 42, 0]);
  });
  ...
  it("add/sub/scale/dot/magnitude/normalized", () => {
    ...
    expect(Array.from(add)).toEqual([6, 8, 10, 12]);
    expect(Array.from(sub)).toEqual([-4, -4, -4, -4]);
    expect(Array.from(sc)).toEqual([2, 4, 6, 8]);
  ...
  it("matMul 3x2 x 2x3 -> 3x3", () => {
    ...
    expect(Array.from(p)).toEqual([1 * 7 + 2 * 10, 1 * 8 + 2 * 11, 1 * 9 + 2 * 12, 3 * 7 + 4 * 10, 3 * 8 + 4 * 11, 3 * 9 + 4 * 12, 5 * 7 + 6 * 10, 5 * 8 + 6 * 11, 5 * 9 + 6 * 12]);
  });
  ...
  it("BigInt fromValues with mixed inputs and ops", () => {
    const m = BigInt64Matrix2x2.fromValues([1n, 2, "3", 4]);
    const n = BigInt64Matrix2x2.fromValues([5, 6, 7, 8]);
    const add = m.add(n);
    const sc = m.scale(2);
    expect(Array.from(m)).toEqual([1n, 2n, 3n, 4n]);
    expect(Array.from(add)).toEqual([6n, 8n, 10n, 12n]);
    expect(Array.from(sc)).toEqual([2n, 4n, 6n, 8n]);
  });
