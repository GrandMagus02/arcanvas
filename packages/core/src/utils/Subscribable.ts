...
export class Subscribable<T extends EventMap | string = Record<string, unknown[]>> {
   ...
   */
  readonly events: EventBus<T extends string ? StringToEventMap<T> : T> = new EventBus<
    T extends string ? StringToEventMap<T> : T
  >();
   ...
   */
  on<K extends keyof (T extends string ? StringToEventMap<T> : T)>(
    event: K,
    fn: (...args: (T extends string ? StringToEventMap<T> : T)[K]) => void,
  ): () => void {
   ...
   */
  off<K extends keyof (T extends string ? StringToEventMap<T> : T)>(
    event: K,
    fn: (...args: (T extends string ? StringToEventMap<T> : T)[K]) => void,
  ): void {
   ...
   */
  emit<K extends keyof (T extends string ? StringToEventMap<T> : T)>(
    event: K,
    ...args: (T extends string ? StringToEventMap<T> : T)[K]
  ): void {
