...
import { isShaderProvider, type UniformContext } from "../shaders/ShaderProvider";
...
export class WebGLBackend implements IRenderBackend {
  ...
  prepareMaterial(_material: BaseMaterial): void {
    void _material; // No-op for now. Hook for textures/pipelines.
  }

  drawMesh(args: DrawArgs): void {
    const { material } = args;

    // Check if material provides its own shaders (ShaderProvider interface)
    if (isShaderProvider(material)) {
      this.drawWithCustomShader(args);
      return;
    }

    // Fall back to default shader based on shading model
    this.drawWithDefaultShader(args);
  }

  /**
   * Draw using custom shader provided by the material.
   */
  private drawWithCustomShader(args: DrawArgs): void {
    ...
    // Get or create program for this shader
  ...
  /**
   * Draw using default shaders from the registry.
   */
  ...
  private getProgramInfo(material: BaseMaterial, mesh: Mesh): ProgramInfo {
    ...
    // Try to get shader from registry
