...
import type { RasterSourceRef } from "./RasterSource";
...
export interface LayerMask {
  source: unknown; // RasterSourceRef | VectorMaskRef - abstracted for now
  inverted: boolean;
}
...
export class RasterLayer extends BaseLayer {
  ...
  source: RasterSourceRef | null = null;
...
export class VectorLayer extends BaseLayer {
  override name = "Vector";
  shapes: unknown[] = []; // VectorShape[] - to be defined in Phase 3
...
export class TextLayer extends BaseLayer {
  override name = "Text";
  text = "";
  style: Record<string, unknown> = {}; // TextStyle - to be defined later
...
export class AdjustmentLayer extends BaseLayer {
  ...
  adjustmentType: string = ""; // "curves" | "levels" | "hue-sat" | ...
